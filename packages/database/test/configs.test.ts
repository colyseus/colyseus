...
async function newDb(opts: { presence?: LocalPresence; registry?: any } = {}): Promise<GameDatabase> {
  const dbPath = path.join(__dirname, `.t-cfg-${Date.now()}-${Math.random().toString(36).slice(2)}.db`);
...
async function cleanupOne(db: GameDatabase): Promise<void> {
  await db.shutdown();
...
describe('ConfigService — typed registry', () => {
  let db: GameDatabase<any, any, typeof registry>;
  beforeEach(async () => { db = await newDb({ registry }) as any; });
  afterEach(async () => { await cleanupOne(db); });
...
describe('ConfigService — in-memory cache', () => {
  let db: GameDatabase<any, any, typeof registry>;
  beforeEach(async () => { db = await newDb({ registry }) as any; });
  afterEach(async () => { await cleanupOne(db); });
...
describe('ConfigService — subscribe()', () => {
  let db: GameDatabase<any, any, typeof registry>;
  beforeEach(async () => { db = await newDb({ registry }) as any; });
  afterEach(async () => { await cleanupOne(db); });
...
describe('ConfigService — Presence pub/sub', () => {
  it('cross-instance: set() on instance A propagates to instance B', async () => {
    ...
    const dbA = await newDb({ registry, presence }) as GameDatabase<any, any, typeof registry>;
    const dbB = await newDb({ registry, presence }) as GameDatabase<any, any, typeof registry>;
    ...
    try {
      ...
      await cleanupOne(dbA);
      await cleanupOne(dbB);
    }
  });

  it('shutdown unsubscribes from the presence channel', async () => {
    const presence = new LocalPresence();
    const db = await newDb({ registry, presence });
