...
// Each test gets its own sqlite file so plugin lifecycle calls run
// against a clean schema. The `(db as any).__cleanupPath` dance mirrors
// services.test.ts so the unlink path stays predictable.
async function newDb(): Promise<GameDatabase> {
  const dbPath = path.join(__dirname, `.t-plugin-${Date.now()}-${Math.random().toString(36).slice(2)}.db`);
...
describe('AnalyticsPlugin', () => {
  let db: GameDatabase;
  beforeEach(async () => { db = await newDb(); });
...
describe('CloudSavesPlugin', () => {
  ...
  it('loads on join (apply receives the previously-saved payload)', async () => {
    ...
    await db.saves.save(u.id, 0, { hp: 75 });
  ...
  it('saves on leave (payload becomes the persisted row)', async () => {
    ...
    const saved = await db.saves.load(u.id, 0);
