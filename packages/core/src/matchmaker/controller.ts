...
const exposedMethods = ['joinOrCreate', 'create', 'join', 'joinById', 'joinByToken'];
...
export async function invokeMethod(method: string, roomNameOrToken: string, clientOptions: any = {}) {
    if (exposedMethods.indexOf(method) === -1) {
        throw new ServerError(ErrorCode.MATCHMAKE_NO_HANDLER, `invalid method "${method}"`);
    }

    try {
        return await matchMaker[method](roomNameOrToken, clientOptions);
