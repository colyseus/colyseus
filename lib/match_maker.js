...
class MatchMaker {
  ...
  joinById (client, roomId, clientOptions) {
    ...
    room.requestJoin(clientOptions)
    return room
  }
  ...
  joinOrCreateByName (client, roomName, clientOptions) {
    ...
    if (!this.hasHandler(roomName)) {
      throw new Error(`no handler for "${roomToJoin}"`);
    }

    return (this.requestJoin(client, roomName, clientOptions) || this.create(client, roomName, clientOptions))
  }
