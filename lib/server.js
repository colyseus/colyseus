...
class Server extends EventEmitter {
  ...
  onMessage (client, data) {
    ...
    if (typeof(message[0]) === "number" && message[0] == protocol.PING) {
      client.send(msgpack.encode([protocol.PING]), { binary: true })

    } else if (typeof(message[0]) === "number" && message[0] == protocol.JOIN_ROOM) {
      try {
