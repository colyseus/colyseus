...
class Room extends EventEmitter {
  ...
  send (client, message) {
    client.send( msgpack.encode( message ) )
  }

  sendState (client) {
    client.send( msgpack.encode( [ protocol.ROOM_STATE, this.roomId, this.state ] ) )
  }
  ...
  broadcast (data) {
    ...
    if (!(data instanceof Buffer)) { data = msgpack.encode(data) }
