...
class Room extends EventEmitter {
  ...
  constructor (options, initialState) {
    ...
    if (options.updateInterval && this.update) {
      this.updateInterval = setInterval(this.update.bind(this), options.updateInterval)
    }
  }
  ...
  sendState (client) {
    client.send( msgpack.encode( [ protocol.ROOM_STATE, this.roomId, this.state ] ), { binary: true } )
  }
